...
- UART/semihosting/exit condition 캡처와 success criteria 기반 exit code
...
- 구버전 산출물 read-time compatibility layer
- PDF 데이터시트를 RAG용 markdown/text로 변환하는 `ingest-pdf` 명령
- 표준 라이브러리 HTTP 기반 최소 GUI와 job 로그/취소/재시도/산출물 조회 API
- Docker 기반 개발 환경
...
docker compose run --rm firmware-mvp firmware-mvp analyze samples/demo_firmware.bin --device stm32f1 --out /output/docker-demo
...
저장됩니다. `shell` 서비스도 repo mount는 read-only이므로 실험 산출물은 `/output` 또는 `/tmp`에
저장하세요.
...
컨테이너 안에서 PDF ingestion을 사용하려면 `pdftotext`가 필요합니다. 기본 Dockerfile에는
포함하지 않았으므로, `poppler-utils`를 추가한 파생 이미지에서 `ingest-pdf`를 실행하거나 로컬
환경에서 실행하세요.
...
`ingest-pdf`는 `pdftotext -layout`으로 PDF를 텍스트화하고, RAG 검색 대상이 되는 `.txt`와
출처 메타데이터가 포함된 `.md`를 생성합니다. 생성된 파일은 `references/` 아래에 두면 기존
`infer`, `analyze`, `run`의 SQLite 기반 문서 검색에 자동 포함됩니다.
...
`pdftotext`가 없으면 명확한 오류/skip 사유를 반환하며, `--pdftotext /path/to/pdftotext`로 실행
파일을 고정할 수 있습니다.
...
GUI는 별도 프레임워크 없이 표준 라이브러리 `http.server`로 실행됩니다.

```bash
firmware-mvp gui --host 127.0.0.1 --port 8765
```

브라우저에서 실행 파라미터를 입력해 `analyze`, `extract`, `infer`, `emulate`, `loop`,
`report`, `feedback`, `validate`, `init-sample`, `ingest-pdf`를 실행할 수 있습니다. GUI job은
내부적으로 `python -m firmware_mvp.cli ...`를 호출하므로 CLI와 산출물 계약을 공유합니다.
작업 상태/로그 조회, 취소, 재시도, 주요 산출물(`context.json`, `inference.json`,
`emulator_config.json`, `emulation_result.json`, `loop_summary.json`, `report.md`) 조회 API를
제공합니다. 성공한 실행에서 `report.md`가 있으면 GUI가 생성한 command도 report에 다시 기록합니다.
...
이 버전은 정적 분석, 선택적 Ghidra/IDA Headless, deterministic/mock/OpenAI 추론, stub/Qiling 실행
백엔드를 하나의 산출물 계약으로 묶은 로컬 MVP입니다. 외부 도구가 없어도 휴리스틱/stub 경로로
파이프라인과 산출물 형식을 검증할 수 있습니다. QEMU는 `docs/qemu-adapter-design.md`로 adapter
계약을 고정했으며, 실제 `--backend qemu` 실행기는 후속 구현 범위입니다.
